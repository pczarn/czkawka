...
fn create_dialog_non_group(window_main: &gtk4::Window) -> Dialog {
    let dialog = gtk4::Dialog::builder().title("Invalid selection with some groups").transient_for(window_main).modal(true).build();
    ...
    let label: gtk4::Label = gtk4::Label::new(Some(&fl!("hard_sym_invalid_selection_label_1")));
    let label2: gtk4::Label = gtk4::Label::new(Some(&fl!("hard_sym_invalid_selection_label_2")));
    let label3: gtk4::Label = gtk4::Label::new(Some(&fl!("hard_sym_invalid_selection_label_3")));
...
fn create_dialog_ask_for_linking(window_main: &gtk4::Window) -> (Dialog, CheckButton) {
    let dialog = gtk4::Dialog::builder().title(&fl!("hard_sym_link_title_dialog")).transient_for(window_main).modal(true).build();
    let button_ok = dialog.add_button(&fl!("general_ok_button"), ResponseType::Ok);
    dialog.add_button(&fl!("general_close_button"), ResponseType::Cancel);

    let label: gtk4::Label = gtk4::Label::new(Some(&fl!("hard_sym_link_label")));
