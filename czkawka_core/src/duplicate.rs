...
impl DuplicateFinder {
    ...
    fn check_files_name(&mut self, stop_receiver: Option<&Receiver<()>>, progress_sender: Option<&futures::channel::mpsc::UnboundedSender<ProgressData>>) -> bool {
        ...
        let dir_traversal = DirTraversalBuilder::new()
            .root_dirs(&self.directories.included_directories[..])
            .stop_receiver(stop_receiver)
            .progress_sender(progress_sender)
            .group_by(|entry_data| entry_data.file_name().to_string_lossy().to_string())
            .build();
        dir_traversal.run();
